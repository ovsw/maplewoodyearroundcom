...
- **App structure:** Next.js App Router with route groups. Frontend routes live in `src/app/(frontend)`, embedded Studio routes live in `src/app/(studio)`.
- **Studio URL:** Sanity Studio is mounted at `/admin` via `src/app/(studio)/admin/[[...tool]]/page.tsx`.
- **Package manager:** Prefer `pnpm` commands in this repository.
- **Generated files:** `src/sanity/types.ts` is generated by TypeGen. Never edit it manually.
- **After schema/query edits:** Run `pnpm typegen` to refresh generated Sanity types.
- **Slug conventions:** Routable docs (`page`, `blog.post`) use `metadata.slug.current`; `blog.category` uses `slug.current`.
- **Validation commands:** Use `pnpm lint` and `pnpm typecheck` to validate changes.

## Module Integration Checklist

When adding a new module type, update all of these:

1. Add the schema under `src/sanity/schemaTypes/modules`.
2. Register it in `src/sanity/schemaTypes/index.ts`.
3. Add it to the modules array in `src/sanity/schemaTypes/fragments/modules.ts`.
4. Add the UI renderer mapping in `src/ui/modules/index.tsx`.
5. Extend GROQ projections in `src/sanity/lib/queries.ts` if module-specific fields need expansion.
